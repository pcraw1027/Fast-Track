class CroupierCore::UpgradePitLevel < ApplicationService
  def call(barcode:, product_id:)
    ...
    if cit_rec && cit_rec.level == 0
      cit_rec.update(product_activity_count: cit_rec.product_activity_count + 1,
      product_orphan_count: cit_rec.product_orphan_count + 1) if cit_rec
