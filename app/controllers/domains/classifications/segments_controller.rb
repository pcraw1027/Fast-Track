class Domains::Classifications::SegmentsController < ApplicationController
  ...
  def create
    @segment = Domains::Classifications::Segment.new(segment_params)
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to domains_classifications_segments_path, status: :see_other, notice: "Segment was successfully destroyed." }
  ...
  private
    ...
    def set_segment
      @segment = Domains::Classifications::Segment.find(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def segment_params
      params.require(:domains_classifications_segment).permit(:code, :title, :description, :product_category_source_id)
    end
end
