class Api::V1::Auth::SessionsController < Api::V1::BaseController
  ...
  def show 
    render json: { user_profile: Domains::Users::V1::UserSerializer.new(current_user).serializable_hash[:data][:attributes] }, 
status: :ok
  end
  ...
  def destroy
    ...
    if jwt_payload && Domains::Users::AllowlistedJwt.find_by(jti: jwt_payload[0]['jti'])
      Domains::Users::AllowlistedJwt.find_by(jti: jwt_payload[0]['jti']).destroy
  ...
  def destroy_all
    if current_user
      Domains::Users::AllowlistedJwt.where(user_id: current_user.id).destroy_all
