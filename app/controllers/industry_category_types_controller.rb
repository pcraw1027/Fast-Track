class IndustryCategoryTypesController < ApplicationController
  ...
  def index
    @industry_category_types = IndustryCategoryType.all.paginate(page: params[:page], per_page: 20).order(
  ...
  def search
    if params[:q].present?
      query = "%#{params[:q]}%"
      companies = IndustryCategoryType.where("title ILIKE ? OR category_code ILIKE ?", query, query).limit(20)
    else
      companies = IndustryCategoryType.none
    end
