class PersonSocialSitesController < ApplicationController
  before_action :set_person_social_site, only: %i[ edit update destroy ]

  # GET /person_social_sites or /person_social_sites.json
  def index
    @person_social_sites = PersonSocialSite.includes(:social_site, :person).all
  end

  # GET /person_social_sites/1 or /person_social_sites/1.json
  def show
    @person_social_site = PersonSocialSite.includes(:social_site, :person).find(params[:id])
  end

  # GET /person_social_sites/new
  def new
    @person_social_site = PersonSocialSite.new
  end
  ...
  def create
    @person_social_site = PersonSocialSite.new(person_social_site_params)
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to person_social_sites_path, status: :see_other, notice: "Person social site was successfully destroyed." }
  ...
  private
    ...
    def set_person_social_site
      @person_social_site = PersonSocialSite.find(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def person_social_site_params
      params.require(:person_social_site).permit(:person_id, :social_site_id, :profile_link)
    end
end
